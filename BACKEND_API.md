...
7. **Vercel logs: `DEP0169` / `url.parse()`** — Express still pulls in **`parseurl`**, which calls Node’s legacy **`url.parse()`** in some code paths. Node prints **`[DEP0169] DeprecationWarning`**. It is **not** a CVE alert for your app logic. This repo sets **`vercel.json` → `env.NODE_OPTIONS`** to **`--disable-warning=DEP0169`** so function logs stay readable. If you already set **`NODE_OPTIONS`** in the Vercel dashboard, **append** the same flag there (dashboard values can override `vercel.json`).
